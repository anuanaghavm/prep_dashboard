...
const MetaTagsBlogForm = () => {
  ...
  const fetchMetaTags = async () => {
    try {
      const response = await allaxios.get(API_URL.META_TAG_BLOGES.GET_ALL);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await allaxios.delete(API_URL.META_TAG_BLOGES.DELETE(id));
      fetchMetaTags();
    } catch (error) {
    ...
    try {
      if (editData?.id) {
        await allaxios.patch(
          API_URL.META_TAG_BLOGES.UPDATE(editData.id),
          payload,
          {
        ...
        await allaxios.post(API_URL.META_TAG_BLOGES.CREATE, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
      fetchMetaTags();
